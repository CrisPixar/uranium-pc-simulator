#!/usr/bin/env python3
"""Проверка v5 (b7): откат PrintExpert/маркета, Supports ×1/2/4/8,
гирлянды у потолка, подарки, стекло, версия."""
...
# подарки: объекты Gift в сценах, все ссылки валидны
import UnityPy
def resolve(fname, fid, pid):
    if pid == 0: return True
    if fid == 0:
        return ctx.obj(fname, pid) is not None
    ext = ctx.files[fname].externals[fid-1]
    base = os.path.basename(ext.path.replace("archive:/", ""))
    return ctx.obj(base, pid) is not None

def scan_pptr(obj, fname, bad):
    if isinstance(obj, dict):
        if set(obj.keys()) == {"m_FileID", "m_PathID"} and obj["m_PathID"]:
            if not resolve(fname, obj["m_FileID"], obj["m_PathID"]):
                bad.append((obj["m_FileID"], obj["m_PathID"]))
        else:
            for v in obj.values(): scan_pptr(v, fname, bad)
    elif isinstance(obj, list):
        for v in obj: scan_pptr(v, fname, bad)

for lv, expect in (("level1", 5), ("level3", 5), ("level4", 5), ("level5", 5), ("level6", 2)):
    gifts = []
    for pid, o in ctx.files[lv].objects.items():
        ...
        if g.get("m_Name", "").startswith("Gift"):
            ...
            for c in g["m_Component"]:
                ...
                if not cp: continue
                scan_pptr(ctx.read_tt(lv, cp), lv, bad)
            gifts.append((pid, bad))
    okc = len(gifts) == expect and not any(b for _, b in gifts)
    check(okc, f"{lv}: подарков {len(gifts)}/{expect}, битые ссылки: {[b for _, b in gifts if b][:3]}")
